 ...
 * require-attribution
 *
 * ScienceSwarm custom ESLint rule that enforces decision 3A from the
 * post-pivot plan: any file under `src/` that writes to gbrain via
 * `putPage`, `upsertChunks`, or `addTimelineEntry` must also import
 ...
 * a regression-gate against the failure mode Greptile caught on PR
 * #252 (new write sites that simply forget about attribution), not as
 ...
 *
 * See:
 *   - docs/testing/POST_PIVOT_TEST_PLAN.md Spec 1
 *   - docs/PHASE_B_RESIDUAL_DEBT.md (decision 3A)
 */
...
const rule = {
  meta: {
    ...
    messages: {
      missing:
        "Write site `{{method}}` must thread `getCurrentUserHandle()` from `@/lib/setup/gbrain-installer`. See docs/PHASE_B_RESIDUAL_DEBT.md and decision 3A.",
    },
    schema: [],
  },
  create(context) {
