...
describe("creature-names", () => {
  ...
  it("creatureUsername is Telegram-legal: 5-32 chars, alphanum+underscore, ends in bot", () => {
    const u = creatureUsername("wobblefinch", "alice", "");
    expect(u).toMatch(/^[a-zA-Z0-9_]{5,32}$/);
    expect(u.endsWith("bot")).toBe(true);
  });

  it("creatureUsername accepts a collision suffix", () => {
    const a = creatureUsername("wobblefinch", "alice", "");
    const b = creatureUsername("wobblefinch", "alice", "x9k2");
    expect(a).not.toBe(b);
  ...
  it("creatureUsername strips non-alphanumeric from handle", () => {
    const u = creatureUsername("wobblefinch", "a.l@i_c-e", "");
    // "@" and "-" and "." should be gone; only "alice" remains in the handle slot
    expect(u).toMatch(/^wobblefinch_alice_bot$/);
  });
