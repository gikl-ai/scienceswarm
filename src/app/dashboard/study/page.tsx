...
import {
  buildPaperLibraryHrefForStudySlug as buildPaperLibraryHrefForSlug,
...
function buildCaptureConfirmationMessage(
  ...
  const study = result.study ?? result.project;
  const lines = [
    ...
    study ? `Study: ${study}` : "Study: unlinked",
...
function StudyPageContent() {
  ...
  useEffect(() => {
    ...
    // Reset restore flag when the active study changes, then re-hydrate.
    ...
    try {
      ...
      setInput((currentDraft) => {
        ...
        // lands. Keep that live text unless the study draft key changed.
    ...
    async (signal?: AbortSignal) => {
      ...
      try {
        const res = await fetch(
          `/api/studies/${encodeURIComponent(safeProject)}/import-summary`,
          {
            signal,
          },
        );
  ...
  const appendProjectOrganizerSummary = useCallback(
    async (projectSlug: string, signal?: AbortSignal) => {
      try {
        const res = await fetch(
          `/api/brain/study-organizer?study=${encodeURIComponent(projectSlug)}`,
          { signal },
        );
        if (!res.ok) {
          return;
  ...
  const loadGbrainNodes = useCallback(
    async (signal?: AbortSignal) => {
      ...
      try {
        const res = await fetch(
          `/api/brain/list?study=${encodeURIComponent(activeProjectSlug)}`,
          { signal },
        );
        ...
        const nodes: FileNode[] = pages
          ...
          .filter(({ normalizedType }) => normalizedType !== "study" && normalizedType !== "project")
  ...
  const loadPaperLibraryActivity = useCallback(
    async (signal?: AbortSignal) => {
      ...
      try {
        const res = await fetch(
          `/api/brain/paper-library/scan?study=${encodeURIComponent(activeProjectSlug)}&latest=1`,
          { signal },
        );
        if (signal?.aborted) {
          return;
  ...
  // ── Fetch gbrain pages for the current study ──
  useEffect(() => {
    const controller = new AbortController();
  ...
  // ── Auto-organize uploaded files into the study tree ──
  const handleProjectUpload = useCallback(
    async (files: File[]) => {
      // Upload to the study workspace (disk) and track in chat context.
    ...
    async (
      ...
      if (isRawRenderableKind(kind)) {
        applyPreview({
          ...
          source: "workspace",
          kind,
          rawUrl: buildWorkspaceRawPreviewUrl(path, safeProjectSlug) ?? undefined,
          editable: false,
        });
        return;
      }
      ...
      const sysMessages: Message[] = [
        {
          id: Date.now().toString(),
          role: "system",
          content: `Imported study "${importedFolder.name}" (${importedFolder.totalFiles.toLocaleString("en-US")} files${detectedScope}). Refreshing the organizer summary and study brief...`,
          timestamp: new Date(),
        },
      ];
    ...
    async (text: string) => {
      ...
      setMessages((prev) => [
        ...
        },
        {
          id: replyMessageId,
          role: "assistant",
          content: "Interpreting mixed study evidence...",
          timestamp: new Date(),
        },
      ]);
      setInput("");
        ...
        >
          ...
          >
            ...
            <StudyList
              ...
              onStudyNavigate={() => setMobileProjectListOpen(false)}
            />
          </div>
        </div>
      )}
                ...
                <div className="flex shrink-0 items-center justify-between border-b border-border/70 bg-white/95 px-4 py-3 backdrop-blur-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Link
                      href={
                        activeProjectSlug
                          ? `/dashboard/settings?study=${encodeURIComponent(activeProjectSlug)}`
                          : "/dashboard/settings"
                      }
                    ...
                    <section className="rounded-[28px] border-2 border-border bg-white p-8 shadow-sm">
                      <div className="flex flex-col items-center text-center">
                        ...
                        <p className="mt-2 text-sm text-muted max-w-md">
                          Create a new study and start importing your
                          research, or select an existing study from the
                          study list.
                        </p>
                        <Link
                ...
                <div className="flex-shrink-0 border-t border-rule/70 bg-raised/95 px-6 py-4 backdrop-blur-sm">
                  <div className="mx-auto w-full max-w-[60rem]">
                    <div
                      ...
                      className={`rounded-[var(--radius-3)] border bg-raised shadow-sm transition-colors ${
                        chatInputDragOver
                      ...
                      <div className="px-4 pb-2 pt-4">
                        <div className="relative flex-1">
                          <ChatMentionInput
                            ...
                            aria-label="Chat with your study"
