...
function AudioLevelMeter({ deviceId }: { deviceId: string }) {
    ...
    (async () => {
      try {
        ...
        const draw = () => {
          ...
          canvasCtx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue("--surface-sunk").trim() || "rgb(243 244 246)";
          canvasCtx.fillRect(0, 0, canvas.width, canvas.height);

          const barWidth = (avg / 255) * canvas.width;
          canvasCtx.fillStyle = avg > 10
            ? (getComputedStyle(document.documentElement).getPropertyValue("--ok").trim() || "rgb(34 197 94)")
            : "rgb(209 213 219)";
          canvasCtx.fillRect(0, 0, barWidth, canvas.height / 2 - 1);

          const peakWidth = (peak / 255) * canvas.width;
          canvasCtx.fillStyle = peak > 30
            ? (getComputedStyle(document.documentElement).getPropertyValue("--accent").trim() || "rgb(59 130 246)")
            : "rgb(209 213 219)";
          canvasCtx.fillRect(0, canvas.height / 2 + 1, peakWidth, canvas.height / 2 - 1);

          canvasCtx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue("--text-dim").trim() || "rgb(107 114 128)";
  ...
  return (
    <canvas
      ...
      width={300}
      height={32}
      className="w-full h-8 rounded border border-rule"
    />
  );
}
...
export default function VoiceTestPage() {
  ...
  return (
    <div className="max-w-[640px] mx-auto px-5 py-10 font-sans">
      ...
      {!isSupported && (
        <div className="bg-danger/10 border border-danger/40 p-3 rounded-lg mb-4">
          MediaRecorder not supported in this browser. Use Chrome or Firefox.
        </div>
      )}

      {error && (
        <div className="bg-danger/10 border border-danger/40 p-3 rounded-lg mb-4 flex justify-between">
          <span>{error}</span>
      ...
      {/* Mic selector */}
      <fieldset className="border border-rule rounded-lg p-4 mb-6">
        ...
        <div className="flex gap-2 mb-3">
          <select
            ...
            className="flex-1 px-3 py-2 rounded-md border border-rule text-sm"
          >
            {devices.map((d) => (
          ...
          <button
            ...
            className="px-3 py-2 rounded-md border border-rule bg-raised cursor-pointer text-sm"
          >
            Refresh
          </button>
        </div>
        {selected && (
          <div>
            <div className="text-xs text-dim mb-1">Live level (speak to see green bars):</div>
      ...
      <fieldset className="border border-rule rounded-lg p-4 mb-6">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-4 py-2 rounded-md border-none text-strong font-semibold ${voiceState === "idle" ? "bg-danger cursor-pointer" : "bg-sunk cursor-not-allowed"}`}
          >
            Start Recording
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-md border-none text-strong font-semibold ${voiceState === "recording" ? "bg-warn cursor-pointer" : "bg-sunk cursor-not-allowed"}`}
          >
            Stop Recording
          </button>
          {voiceState === "speaking" && (
      ...
      <fieldset className="border border-rule rounded-lg p-4 mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-md border-none text-strong font-semibold ${voiceState === "idle" && ttsInput.trim() ? "bg-accent cursor-pointer" : "bg-sunk cursor-not-allowed"}`}
          >
            Speak
          </button>
        </div>
      ...
      <fieldset className="border border-rule rounded-lg p-4">
        <legend className="font-semibold px-2">
          Log
          <button onClick={() => setLog([])} className="ml-2 text-xs cursor-pointer bg-transparent border-none text-dim">clear</button>
        </legend>
