...
function buildProjectPageContent(input: {
  ...
  const frontmatter = [
    ...
    "type: study",
    "para: projects",
    `study: ${JSON.stringify(input.projectSlug)}`,
    `study_slug: ${JSON.stringify(input.projectSlug)}`,
    `legacy_project_slug: ${JSON.stringify(input.projectSlug)}`,
    "tags: [import]",
...
function buildImportPageContent(input: {
  ...
  const frontmatter = [
    ...
    `study: ${JSON.stringify(input.project)}`,
    `study_slug: ${JSON.stringify(input.project)}`,
    `legacy_project_slug: ${JSON.stringify(input.project)}`,
    `source_refs: ${JSON.stringify(input.sourceRefs)}`,
    "status: active",
