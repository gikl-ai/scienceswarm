...
const AUTH_CHALLENGE_PATTERN =
  /\b(auth(?:entication)? required|not authenticated|unauthorized|forbidden|please log in|please login|sign in|login required|api key required|missing api key|invalid api key|failed to authenticate|authentication_error|invalid authentication credentials)\b|\b(?:api error|status|http)\s*:?\s*401\b/i;
