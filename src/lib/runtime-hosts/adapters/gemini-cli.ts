...
export class GeminiCliRuntimeHostAdapter implements ResearchRuntimeHost {
  ...
  constructor(options: GeminiCliRuntimeHostAdapterOptions = {}) {
    ...
    this.command = options.command ?? this.runtimeProfile.transport.command ?? "gemini";
  ...
  async privacyProfile(): Promise<RuntimePrivacyClass | RuntimeHostPrivacyProof> {
    return {
      ...
      reason: "Gemini CLI execution is owned by the user's Google account.",
  ...
  async cancel(sessionId: string): Promise<RuntimeCancelResult> {
    ...
    return {
      ...
      detail: cancelled
        ...
        : "No active Gemini CLI wrapper process was found.",
    };
  }
...
export function createGeminiCliRuntimeHostAdapter(
  options: GeminiCliRuntimeHostAdapterOptions = {},
): GeminiCliRuntimeHostAdapter {
  return new GeminiCliRuntimeHostAdapter(options);
}
