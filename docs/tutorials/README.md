...
| [SYK spectral form factor](syk-spectral-form-factor/README.md) | UI-first ScienceSwarm + Claude Code walkthrough for an exact-diagonalization q=4 SYK ensemble; reproduces the dip–ramp–plateau and Wigner–Dyson level statistics, with Study artifacts stored under `scripts/` and durable gbrain run logs. | <1 min fast preview, ~3–5 min default |
| [ScienceSwarm SYK pipeline walkthrough](syk-spectral-form-factor-walkthrough.md) | New-user walkthrough for creating a ScienceSwarm Study, importing the SYK tutorial, choosing Claude Code, running the fast preview, inspecting artifacts, and recording interpretation/refinement notes. | reading-time |
