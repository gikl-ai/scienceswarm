...
- `docs/agent-system/SCIENCESWARM_IDENTITY.md` — canonical definition of `ScienceSwarm = OpenClaw + OpenHands + gbrain`, agent roles, and data-flow rules (all data ingested to gbrain first, OpenClaw fronts user comms, OpenHands reads/writes gbrain directly). Wired into AGENTS.md, CLAUDE.md, GEMINI.md, and the agent-system resolver so agents that land in the repo without context know what the system is.
